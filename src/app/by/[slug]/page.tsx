...
export async function generateStaticParams() { return getAllBySlugs().map(slug => ({ slug })); }
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const by = getBy(params.slug); if (!by) return {};
...
export default function BySide({ params }: Props) {
  const by = getBy(params.slug); if (!by) notFound();
  const andreByer = BYER.filter(b => b.slug !== by.slug).slice(0, 8);
  const faqItems = [
    { sporsmal: `Hva koster fasadearbeid i ${by.navn}?`, svar: `Prisene i ${by.navn} følger landsgjennomsnittet. Maling koster 300 til 800 kr per kvm, kledning 1 200 til 3 500 kr per kvm.` },
    { sporsmal: `Hvor finner jeg fasadefirma i ${by.navn}?`, svar: `Gjennom Fasadeteknikk.no kan du enkelt få tilbud fra kvalifiserte fasadefirma i ${by.navn}.` },
    { sporsmal: `Trenger jeg byggetillatelse for fasadearbeid i ${by.navn}?`, svar: `Vanlig vedlikehold krever normalt ikke søknad. Endring kan kreve byggemelding til ${by.navn} kommune.` },
  ];
  return (<><Header /><main>
    ...
    <section className="section-py-sm"><div className="container-site"><div className="cta-block text-center relative z-10"><h2 className="font-display font-extrabold text-display-lg text-white mb-3">Finn fasadefirma i {by.navn}</h2><Link href="#tilbud" className="btn-primary">Få gratis tilbud <ArrowRight className="w-4 h-4" /></Link></div></div></section>
