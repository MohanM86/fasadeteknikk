...
const ARTIKLER = [
  ...
  { slug: "tre-vs-mur-fasade", tittel: "Tre vs mur: Fordeler og ulemper", lesetid: 9 },
  { slug: "hvordan-vedlikeholde-fasade", tittel: "Slik vedlikeholder du fasaden", lesetid: 7 },
];
  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="section-dark"><div className="container-site py-8"><div className="grid grid-cols-2 sm:grid-cols-4 gap-6">{[{ v: "30+", l: "Byer dekket" }, { v: "8", l: "Tjenester" }, { v: "24t", l: "Responstid" }, { v: "100%", l: "Gratis" }].map(({ v, l }) => (<div key={l} className="text-center"><div className="font-display font-extrabold text-4xl text-brand-500 tracking-tight">{v}</div><div className="text-xs text-neutral-500 mt-1">{l}</div></div>))}</div></div></section>
        ...
        <section className="section-light section-py" aria-labelledby="tjenester-h">
          <div className="container-site">
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 mb-12">
              <div><div className="section-label">Tjenester</div><h2 id="tjenester-h" className="font-display font-extrabold text-display-xl text-neutral-900 text-balance">Hva trenger du<br />hjelp med<span className="text-brand-500">?</span></h2></div>
              <p className="text-sm text-neutral-400 max-w-xs lg:text-right">Alt fra enkel fasadevask til komplett rehabilitering. Velg tjeneste for å lære mer.</p>
            </div>
            <ServiceRows />
          </div>
        </section>
        ...
        <section className="section-white section-py-sm" aria-labelledby="prosess-h">
          <div className="container-site"><div className="text-center mb-12"><div className="section-label">Slik fungerer det</div><h2 id="prosess-h" className="font-display font-extrabold text-display-lg text-neutral-900">Tre enkle steg<span className="text-brand-500">.</span></h2></div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">{PROSESS.map(s => (<div key={s.nr} className="step-card"><div className="step-num">{s.nr}</div><h3 className="font-display font-bold text-heading-md text-neutral-900 mb-2">{s.tittel}</h3><p className="text-sm text-neutral-500 leading-relaxed">{s.tekst}</p></div>))}</div>
          </div>
        </section>

        <section className="section-light section-py-sm" aria-labelledby="byer-h">
          <div className="container-site">
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 mb-8"><div><div className="section-label">Dekning</div><h2 id="byer-h" className="font-display font-extrabold text-display-lg text-neutral-900">Finn firma lokalt<span className="text-brand-500">.</span></h2></div><Link href="/by/oslo" className="flex items-center gap-2 text-sm font-semibold text-brand-500 hover:text-brand-600 transition-colors">Se alle byer <ArrowRight className="w-4 h-4" /></Link></div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">{POPULAERE_BYER.map(by => (<Link key={by.slug} href={`/by/${by.slug}`} className="card-flat px-5 py-4 flex items-center justify-between group"><div><div className="font-display font-semibold text-base text-neutral-800 group-hover:text-brand-500 transition-colors">{by.navn}</div><div className="text-xs text-neutral-400 mt-0.5">{by.innbyggere.toLocaleString("nb-NO")} innb.</div></div><ArrowUpRight className="w-4 h-4 text-neutral-300 group-hover:text-brand-500 transition-colors" /></Link>))}</div>
          </div>
        </section>

        <section className="section-white section-py-sm" aria-labelledby="artikler-h">
          <div className="container-site">
            <div className="text-center mb-8"><div className="section-label">Artikler</div><h2 id="artikler-h" className="font-display font-extrabold text-display-lg text-neutral-900">Lær mer om fasade<span className="text-brand-500">.</span></h2></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{ARTIKLER.map(a => (<Link key={a.slug} href={`/artikler/${a.slug}`} className="card p-6 group"><h3 className="font-display font-semibold text-heading-sm text-neutral-900 group-hover:text-brand-500 transition-colors mb-2">{a.tittel}</h3><div className="flex items-center gap-2 text-xs text-neutral-400"><Clock className="w-3 h-3" />{a.lesetid} min lesing</div></Link>))}</div>
          </div>
        </section>
