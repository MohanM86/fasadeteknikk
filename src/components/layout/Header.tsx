...
const NAV = [
  ...
  { label: "Finn firma", href: "/fylke", children: [
...
export default function Header() {
  ...
  return (
    <>
      <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300", scrolled ? "bg-neutral-50/95 backdrop-blur-xl border-b border-neutral-200/50 shadow-sm" : "bg-transparent")}>
        <div className="container-site">
          <div className="flex items-center justify-between h-[72px]">
            ...
            <button onClick={() => setMobileOpen(v => !v)} className="lg:hidden p-2 rounded-8 hover:bg-neutral-100 transition-colors">
          ...
          <nav className="px-4 py-4 space-y-1">
            {NAV.map(item => (
              <div key={item.label}>
                ...
                {item.children && <div className="ml-4 mt-1 space-y-1 mb-2">{item.children.map(child => (<Link key={child.href} href={child.href} className="block px-3 py-2 rounded-8 text-sm text-neutral-500 hover:bg-neutral-50 hover:text-brand-500" onClick={() => setMobileOpen(false)}>{child.label}</Link>))}</div>}
              </div>
            ))}
          </nav>
